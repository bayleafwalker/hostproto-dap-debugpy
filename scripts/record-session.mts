// Record one real debugger session (debugpy) as an NDJSON envelope log for an EvidenceSet consumer.
...
const p = await call('hostproto_context_create', { program: 'fixtures/program.py', cwd: ROOT, client: { id: 'vuoro-evidence-recorder' } });
const s1 = p.surface.id;
await call('hostproto_surface_act', intent(s1, 'a-bp', 'set_breakpoints', { params: { source: 'fixtures/program.py', lines: [16] }, declared_effects: ['breakpoints_replaced_for_source'] }));
...
const h = await call('hostproto_context_create', { program: 'fixtures/spin.py', cwd: ROOT, client: { id: 'vuoro-evidence-recorder' } });
