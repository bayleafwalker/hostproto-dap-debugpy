...
import { DebugpyHost, HostProtoError, PROJECTIONS, type Artifact } from './host.js';
import { anyOf, toolSchema, pinnedCommit, assertValid } from './schemas.js';

export const SERVER_INFO = { name: 'hostproto-dap-debugpy', version: '0.0.1' };
export const PROTOCOL_REVISION = '2026-07-28';
const stateUri = (surface: string) => `hostproto://surface/${surface}/state`;

export function createServer(host: DebugpyHost): McpServer {
  const server = new McpServer(SERVER_INFO, {
    capabilities: { tools: {}, resources: { subscribe: true, listChanged: true } },
    instructions: `HostProto dap/v1 adapter on debugpy. Schemas pinned to hostproto-semantics@${pinnedCommit.slice(0, 12)}. Launch a program (stops on entry), then observe/act/await on the thread surface. Frame, scope and variable targets are valid for one revision only; every stop or resume moves it.`,
  });
  ...
  server.registerTool('hostproto_context_create', {
    title: 'Launch a program under debugpy',
    description: 'Mint host, context and surface handles for a launched Python program. Stops on entry by default. console=integratedTerminal makes the adapter ask the client to run the debuggee (runInTerminal), surfaced as a host request with a decision token. Returns hostproto.handles/v1.',
    inputSchema: fromJsonSchema({ type: 'object', required: ['program'], additionalProperties: false, properties: {
      program: str, args: { type: 'array', items: { type: 'string' } }, cwd: { type: 'string' }, console: { enum: ['internalConsole', 'integratedTerminal'] }, stop_on_entry: { type: 'boolean' },
