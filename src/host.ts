// The debugpy host: the only place that speaks DAP. Everything it returns is
...
export class DebugpyHost {
  ...
  constructor(private readonly python = DEFAULT_PYTHON) {}
  ...
  // -- handles --------------------------------------------------------------
  async createContext(params: { program: string; args?: string[]; cwd?: string; console?: string; stop_on_entry?: boolean; client?: { id?: string } }) {
    ...
    const console_ = params.console ?? 'internalConsole';
    if (!['internalConsole', 'integratedTerminal'].includes(console_)) throw new HostProtoError('capability_unsupported', 'console must be internalConsole or integratedTerminal', false, { console: console_ });
    const client = DapClient.spawn(this.python, ['-m', 'debugpy.adapter'], cwd);
    ...
    try {
      ctx.capabilities = await client.request('initialize', { clientID: 'hostproto', clientName: 'hostproto-dap-debugpy', adapterID: 'debugpy', pathFormat: 'path', linesStartAt1: true, columnsStartAt1: true, supportsRunInTerminalRequest: true, locale: 'en' }, 15000);
      this.adapterIdentity = { dap: 'debugpy.adapter', python: this.python, supportsStepBack: ctx.capabilities.supportsStepBack ?? false, supportsRestartFrame: ctx.capabilities.supportsRestartFrame ?? false };
      const initialized = new Promise<void>(resolve => { const off = client.onEvent(e => { if (e.event === 'initialized') { off(); resolve(); } }); });
      const launch = client.request('launch', { type: 'python', request: 'launch', name: 'hostproto', program, args: params.args ?? [], cwd, console: console_, stopOnEntry: params.stop_on_entry ?? true, justMyCode: true, redirectOutput: true, python: this.python }, 120000);
      launch.catch(() => {});
      ...
      if (console_ === 'internalConsole') {
        ...
        finish.catch(error => { if (main.lifecycle === 'creating' || main.lifecycle === 'open') { main.lifecycle = 'terminated'; ctx.terminated = true; this.emit(main, 'launch.failed', { message: String((error as Error).message) }); } });
      }
    } catch (error) {
  ...
  async act(intent: J) {
    ...
    try {
      if (kind === 'set_breakpoints') {
        ...
        while (!(surface.state.stopped && surface.events.some(e => e.seq > seqAtSend && e.kind === 'stopped')) && surface.lifecycle === 'open' && Date.now() < end) await new Promise(r => setTimeout(r, 15));
        ...
        else if (surface.state.stopped && surface.events.some(e => e.seq > seqAtSend && e.kind === 'stopped')) {
          const top = await this.topFrame(ctx, surface).catch(() => undefined);
  ...
  capabilityProfile() {
    ...
    const profile = {
      ...
      adapter: { kind: 'debugpy', variant: 'python', identity: this.adapterIdentity }, immutable_per_run: true,
      capabilities: Object.fromEntries([
        ...
        ...PROJECTIONS.map(p => cap(`observe.${p}`, 'supported', p === 'host_requests' || p === 'breakpoints' ? 'host' : 'engine', p === 'state' ? 'normalized' : 'exact')),
        ...INTENT_FAMILY.map(k => cap(`act.${k}`, 'supported', k === 'host_request.resolve' ? 'host' : 'engine', 'exact')),
